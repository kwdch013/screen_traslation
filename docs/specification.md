...
- パイプラインは `crop_revision` の変更を検知した時点で OCR の安定化状態と重畳フィードバック判定用テキストを破棄し、旧クロップの領域を新しいリビジョンの結果へ持ち越さない。
...
- 設定・辞書ごとに同一ディレクトリのロックファイルを使い、Windows では `msvcrt.locking`、Linux では `fcntl.flock` によってプロセス間の更新を直列化する。
- 更新時はロック取得後に最新版を再読込し、要求された変更を適用してから、同一ディレクトリの一時ファイルを原子的に置換する。これにより、同じ端末上の Web API と CLI による先行更新を保持する。
- CLI の `--add-term` はロック内の最新版へ指定された1件だけを追加または上書きする。`--run-once` と `--text` は既存の設定・辞書を保存し直さず、未作成の場合だけロック内で初期ファイルを作成する。
- ファイルロックを2秒以内に取得できない設定・辞書の更新 API は、対象ロックファイルを示すメッセージと HTTP 503 を返す。
