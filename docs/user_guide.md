...
- 共有中のプレビュー上をドラッグすると、矩形内だけを翻訳対象にできます。`選択範囲を解除` を押すと画面全体の翻訳へ戻ります。
- 選択範囲はブラウザに保存され、次回共有時に動画の解像度が同じ場合だけ復元されます。解像度が変わった場合は画面全体へ戻るため、必要に応じて選択し直してください。
- LLM OCR が画像内の位置を返した訳文も共有映像上へ重ねます。応答を解釈できない場合は位置を推測せず、共有映像の下に表示します。
...
設定と辞書は、同じ端末の Web 画面と CLI から同時に更新できます。更新はファイルロックで直列化され、保存直前に読み直した最新版へ要求された変更だけが適用されます。`--run-once` と `--text` は既存の設定・辞書を保存し直さず、ファイルが無い初回実行時だけ初期ファイルを作成します。ロックを2秒以内に取得できない場合、Web 画面の更新 API は対象ファイルを示すメッセージと HTTP 503 を返すため、少し待ってから再実行してください。複数マシン間やネットワークファイルシステム上の同期は保証されません。
