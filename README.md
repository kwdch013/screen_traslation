...
- プレビュー上のドラッグによる翻訳対象領域のクロップと、同一解像度での選択復元
- ブラウザからの翻訳設定と用語辞書の編集
...
設定は `config/app.json`、辞書は `config/glossary.json` に保存されます。同じ端末の Web API と CLI が同時に更新した場合も、ファイルロック内で最新版を再読込して要求された設定変更・辞書の登録または削除だけを適用するため、先行更新は保持されます。`--run-once` と `--text` は既存ファイルを保存し直さず、ファイルが無い初回実行時だけロック内で初期ファイルを作成します。ロックを2秒以内に取得できない更新 API は、対象ファイルを示すメッセージと HTTP 503 を返します。
